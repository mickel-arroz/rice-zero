 ...
 * en la ventana en la que la sesión aún no está puesta, o sea, en la primera
 * petición hecha con un token recién traído. A partir de ahí la conexión ya la
 * tiene, y una lista vacía es una lista vacía.
 *
...
export type EmptyRead = {
  /** Cuántas filas trajo. Cero es lo sospechoso, y solo cero. */
  rows: number;
 ...
 *
 *   · **Cero filas.** Con filas no hubo tropiezo: RLS casó.
...
export function shouldRetryEmptyRead({
  ...
  return rows === 0 && tokenIsFresh && !retried;
}
